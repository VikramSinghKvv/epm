...
module.exports = function () {
  ...
  this.on('getTotal', 'Orders', async (req) => {
    const { ID } = req.params[0];
    console.log(`Calculating total for Order ID: ${ID}`);

    const items = await SELECT.from('com.epm.SalesOrderItems')
      .where({ order_ID: ID });
      console.log(`Found ${items.length} items for Order ID: ${ID}, items: ${JSON.stringify(items)}`);
  ...
  this.on('bulkConfirm', async (req) => {
    ...
    const { SalesOrders } = cds.entities;
    console.log(`Bulk confirming orders: ${orderIds}`);
    console.log(`Received order IDs: ${JSON.stringify(orderIds)}, ${SalesOrders}`);
